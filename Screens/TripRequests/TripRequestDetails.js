...
export default class BorrowerRequestDetails extends Component {
  ...
  canReportOwnerNoShow = () => {
    ...
    var isDayOfTrip = currentDate.toDateString() == bookedDate.toDateString();
    var isLaterThan3PM = currentDate.getHours() >= 15;
    var isNotCheckedIn = this.state.currentRequest.status == 'confirmed'
  ...
  handleCancelRequest = (automatically) => {
    ...
    if (automatically) {
      ...
      batch.update(trip, { status: 'cancelled', cancellationReason: 'session-expired'});
      ...
      batch.update(borrowerRequest, { status: 'cancelled' ,cancellationReason: 'session-expired'});

      var ownerRequest = database.collection('users').doc(this.state.currentRequest.ownerID)
        .collection('Requests').doc(this.state.currentRequest.tripID);
      batch.update(ownerRequest, { status: 'cancelled' ,cancellationReason: 'session-expired'});
          ...
          {this.state.canReportOwnerNoShow ? <View>
            <TouchableOpacity
               onPress={()=> {
                Alert.alert(
                    "إلغاء الطلب",
                    "سيتم إلغاء الطلب في حال عدم عدم حضور المؤجر لموقع التسليم، هل انت متأكد من إلغاء الطلب؟",
                    [
                        {text:'لا',
                      style:'default',
                  },{
